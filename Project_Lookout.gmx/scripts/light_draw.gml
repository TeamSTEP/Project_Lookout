        ...
        with (_light) {
            ...
            if (_light_inside) {
                if (!_light_shadows) {
                    ...
                    draw_set_blend_mode(bm_add);
                    if (surface_exists(global._light_surface)){draw_surface(global._light_surface, (x - global._light_half_size - argument0) * global._light_quality, (y - global._light_half_size - argument1) * global._light_quality);}
                    draw_set_blend_mode(bm_normal);
                    surface_reset_target();
                }
            }
...
if (!argument2){draw_set_blend_mode_ext(bm_dest_color, bm_zero);}
else{draw_set_blend_mode_ext(bm_dest_color, bm_src_color);}
